document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 1. Récupérer l'ID de l'utilisateur connecté (crucial pour la logique des likes)
    const currentUserId = String(localStorage.getItem('twitterCloneUserId'));
    if (!currentUserId) {
        // Rediriger si personne n'est connecté, car on ne peut pas liker
    ...
    const handleLikeClick = async (likeBtn) => {
        ...
        try {
            ...
            // Mettre à jour le compteur de likes sur l'interface
            ...
            // En cas d'erreur, on pourrait recharger les tweets pour être sûr
        }
    };
    ...
    // Fonction pour afficher les tweets (MAINTENANT AVEC LA LOGIQUE DE LIKE)
    const displayUserTweets = (tweets, author) => {
        ...
        tweets.forEach(tweet => {
            ...
            tweetEl.dataset.tweetId = tweet.id; // Ajout de l'ID ici

            // Logique de like, identique à accueil.js
            const likedBy = tweet.likedBy || [];
            const isLikedByCurrentUser = likedBy.includes(currentUserId);
            ...
            tweetEl.innerHTML = `
                ...
                <div class="tweet-body">
                    ...
                    <!-- Actions de tweet mises à jour pour correspondre à accueil.js -->
                    <div class="tweet-actions">
                        <button class="tweet-action reply-btn" data-tweet-id="${tweet.id}">
    ...
    try {
        // Lancer les requêtes en parallèle pour plus de rapidité
        ...
        // Ajouter l'écouteur d'événements pour les clics sur les tweets
        const tweetsContainer = document.getElementById('tweets-section');
